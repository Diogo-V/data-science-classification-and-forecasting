...
class PersistenceRegressor (RegressorMixin):
    ...
    def explore_rolling_mean_regressor(self):
      ...
      f = open(f'climate-forecasting/records/evaluation/climate_persistence_results.txt', 'w')
      f.write(f'{eval_results}')

      plot_evaluation_results(self.train_data.values, prd_trn, self.test_data.values, prd_tst, 'Persistence', f'climate-forecasting/records/evaluation/climate_persistence_eval')
      self.plot_forecasting_series(self.train_data, self.test_data, prd_trn, prd_tst, 'Persistence', f'climate-forecasting/records/evaluation/climate_persistence_plots.png', x_label='timestamp', y_label='QV2M')
