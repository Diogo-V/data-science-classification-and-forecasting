...
class RollingMeanRegressor (RegressorMixin):
    ...
    def compute_rolling_mean_regressor(self):
      ...
      f = open(f'climate-forecasting/records/evaluation/climate_rolling_mean_best_{self.best_size}_results.txt', 'w')
    ...
    def explore_rolling_mean_regressor(self):
      ...
      for size in self.win_sizes:
        ...
        eval_results['RollingMean'] = PREDICTION_MEASURES[measure](self.test_data.values, prd_tst)
        f = open(f'climate-forecasting/records/evaluation/climate_rolling_mean_{size}_results.txt', 'w')
