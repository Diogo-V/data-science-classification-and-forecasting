  ...
  def explore_scatter_plot(self, output_image_path: str) -> None:
    ...
    for i in range(50, 52):
        ...
        for j in range(i+1, len(columns)):
            var2 = columns[j]
            axs[i - 50, j-1].set_title("%s x %s"%(var1,var2))
            axs[i - 50, j-1].set_xlabel(var1)
            axs[i - 50, j-1].set_ylabel(var2)
            axs[i - 50, j-1].scatter(self.data[var1], self.data[var2])
    savefig(f'{output_image_path}/sparsity_study_5.png')
